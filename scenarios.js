...
const SC_LABEL = { Downside: '悲觀', Base: '基本', Upside: '樂觀' };
const SC_EN = { Downside: 'Downside', Base: 'Base', Upside: 'Upside' };
...
function renderCards() {
  ...
  const delta = d => d == null ? '' : `<div style="color:${d >= 0 ? 'var(--pos)' : 'var(--bpm-red)'};font-size:11px;margin-top:2px;">${d >= 0 ? '▲ +' : '▼ '}${fmtC(Math.abs(d))} vs 基本</div>`;
  ...
  host.innerHTML = SC.map(s => {
    ...
    return `<div class="card sc-card${sel ? ' sel' : ''}" data-sc="${s}" style="cursor:pointer;">
      <div class="eyebrow" style="color:${SC_COLOR[s]};">${SC_LABEL[s]} · ${SC_EN[s]}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
        ${cell('期末組合估值', fmtC(v), s === 'Base' ? null : v - baseV)}
        ${cell('期末年度 NOI', fmtC(n), s === 'Base' ? null : n - baseN)}
        ${cell('期末淨現金流', fmtC(c), s === 'Base' ? null : c - baseC)}
        ${cell('預估 DSCR', dscr != null ? dscr.toFixed(2) + 'x' : '—', null)}
      </div>
    </div>`;
  }).join('');
...
function buildOpt(data) {
  const series = SC.map(s => {
    ...
    return {
      name: SC_LABEL[s], type: 'line', data: data[s], smooth: true, symbol: 'none',
...
function renderDrivers() {
  ...
  host.innerHTML = `
    <div style="background:var(--neg-tint);border:1px solid var(--rule);border-left:2px solid var(--bpm-red);padding:12px 14px;margin-bottom:14px;">
      <div class="eyebrow eyebrow-red" style="margin-bottom:4px;">最迫切 · Top concern</div>
      <div style="font-family:var(--font-cjk);font-size:13.5px;line-height:1.75;color:var(--ink-2);">${escHtml(rs.topConcern || '—')}</div>
    </div>
    ${highs.length ? '<div class="eyebrow" style="margin-bottom:6px;">高風險項目 · High risks</div>' + highs.map(h =>
      `<div class="row-between" style="border-bottom:1px solid var(--rule);padding:9px 0;gap:10px;">
        ...
        <span style="display:inline-flex;gap:8px;align-items:center;white-space:nowrap;"><span class="meta">${escHtml(h.asset)}</span><span class="risk-badge high">高</span></span>
      </div>`).join('') : ''}`;
}
...
function renderContrib() {
  ...
  host.innerHTML = `
    <table class="data-table">
      <thead><tr><th>資產</th><th class="num">基本期末估值</th><th class="num">悲觀期末估值</th><th class="num">下檔幅度</th><th></th></tr></thead>
      <tbody>
        ${rows.map(r => `<tr>
          ...
          <td style="text-align:right;"><a class="doc-link" href="index.html?id=${escHtml(r.code)}">查看風險 ›</a></td>
        </tr>`).join('')}
      </tbody>
    </table>`;
}
